...
test_x=torch.unsqueeze(test_data.test_data,dim=1).type(torch.FloatTensor)[:100]/255.
...
class CNN(torch.nn.Module):
    def __init__(self):
        ...
        self.conv2=torch.nn.Sequential(
            nn.Conv2d(16,32,5,1,2), #input [batch_size,in_channels,height,width] output [batch_size,out_channels,height,width]
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2)
        )
